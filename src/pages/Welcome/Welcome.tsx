...
import { Table, Card, Row, Col, Upload, Button, Input, Form, message, Spin, Select } from 'antd';
...
const Welcome: React.FC = () => {
  ...
  const [recognizedMembers, setRecognizedMembers] = useState<any[]>([]);
  ...
  const parseApiResponse = (response: string): MemberRecognitionResult[] => {
    try {
      ...
      if (Array.isArray(jsonData) && jsonData.length > 0) {
        ...
        if (firstItem['會員ID'] !== undefined && (firstItem['金幣捐獻'] !== undefined || firstItem['活躍貢獻'] !== undefined)) {
          // 將中文鍵名轉換為我們的標準格式
          return jsonData.map(item => ({
            memberId: item['會員ID'] || '',
            contribution: typeof item['金幣捐獻'] !== 'undefined' ? Number(item['金幣捐獻']) : 
                         typeof item['活躍貢獻'] !== 'undefined' ? Number(item['活躍貢獻']) : 0
          }));
        }
      }
...
請對所有圖片中的會員資料進行識別，僅返回JSON格式結果，不要添加任何其他文本或標記。`;
        ...
        try {
          ...
          newProcessingStatus[imageId] = `失敗 (${error instanceof Error ? error.message : '未知錯誤'})`;
          setProcessingStatus(newProcessingStatus);
        }
      }
...
請對所有圖片中的會員資料進行識別，僅返回JSON格式結果，不要添加任何其他文本或標記。`;
  ...
  const determineMemberLevel = (coinsContribution: number, activityContribution: number) => {
    ...
    for (let i = memberLevels.length - 1; i >= 0; i--) {
      ...
      if (coinsContribution >= level.coinsRequired && activityContribution >= level.activityRequired) {
        return level.level;
      }
    }
    return '未達會員標準';
  ...
  // 更新識別後的會員資料
  const updateRecognizedMembers = (coinsData: MemberRecognitionResult[], activityData: MemberRecognitionResult[]) => {
    // 合併金幣和活躍度數據
    const allMemberIds = [...new Set([...coinsData.map(item => item.memberId), ...activityData.map(item => item.memberId)])];
  ...
  // 自訂上傳列表項目渲染
  const customItemRender = (originNode: React.ReactElement, file: UploadFile, fileList: UploadFile[], actions: { download: () => void; preview: () => void; remove: () => void }) => {
    const status = processingStatus[file.uid];
    return (
      <div className="ant-upload-list-item">
        {originNode}
        {status && (
          <div style={{ position: 'absolute', right: '8px', top: '8px', background: status === '完成' ? '#52c41a' : status === '處理中' ? '#1890ff' : '#ff4d4f', color: 'white', padding: '0 8px', borderRadius: '4px' }}>
            {status}
          </div>
        )}
      </div>
  ...
  // 過濾會員函數
  const filterMembersByLevel = (members: any[]) => {
    if (!levelFilter) {
          ...
          <Row gutter={24}>
            <Col span={12}>
              ...
              >
                ...
                {loading1 && <Spin tip="正在進行圖片辨識..." style={{ marginTop: 16, display: 'block' }} />}
              </Card>
            </Col>
            <Col span={12}>
              <Card 
              ...
              >
                ...
                {loading2 && <Spin tip="正在進行圖片辨識..." style={{ marginTop: 16, display: 'block' }} />}
              </Card>
            </Col>
          </Row>
        </Card>
